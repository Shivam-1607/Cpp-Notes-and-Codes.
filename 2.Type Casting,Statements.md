...
-The `continue` statement is used inside loops to **skip the remaining statements of the current iteration** and move directly to the next iteration.
...
-When `i` becomes `3`, `continue` skips `cout` and moves to the next iteration.
...
int main()
{
    for (int i = 1; i <= 10; i++)
    {
        ...
        // Only odd numbers reach this statement
        cout << i << " ";
    }
...
| `break` | `continue` |
|---|---|
| Stops the loop completely | Does not stop the loop |
| Exits the current loop | Skips the current iteration |
| Execution continues after the loop | Execution continues with the next iteration |
| Used to terminate a loop early | Used to skip unwanted iterations |

## 📝 Important Points-

- `continue` works with loops.
  
- It skips the **current iteration**.
  
- The loop continues with the next iteration.
  
- `break` stops the entire loop, while `continue` only skips one iteration.
  
- Be careful with `continue` in `while` loops so that the loop variable is still updated properly.
